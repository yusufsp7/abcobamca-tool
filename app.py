...
# Process user input into a list of keywords
# Split by commas or newlines, then strip whitespace and filter out empty strings
keywords = [
    k.strip() for k in user_keywords_input.replace('\n', ',').split(',') if k.strip()
]

# Display keywords for user reference (now shows what the user entered)
with st.expander("View Current Search Keywords"):
    if keywords:
        st.write("The tool will search for videos using the following keywords:")
        st.code("\n".join(keywords), language="text")
    else:
        st.warning("No keywords entered. Please enter some keywords to search.")
